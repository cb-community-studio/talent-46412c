...
import adminFakerFactory from "./adminFakerFactory";
...
const AdminFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = adminFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("admin").create(elem));
...
export default connect(null, mapDispatch)(AdminFakerDialogComponent);
